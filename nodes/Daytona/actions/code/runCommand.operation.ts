...
const showOnly = { resource: ['code'], operation: ['runCommand'] };
...
export const description: INodeProperties[] = [
	...
	},
	{
		...
		required: true,
		default: '',
		description: 'ID of the existing sandbox to run the command in',
	...
	},
	{
		...
		options: [
			...
			},
			{
				...
				description: 'Maximum time to wait for the command to finish executing',
			},
			{
				...
				description:
					'Max time to wait for the ephemeral sandbox to reach the "started" state before running the command',
			},
		],
	},
];
...
export async function execute(
	...
	const cwd = additional.cwd?.trim() || undefined;
	...
	try {
		result = (await daytonaToolboxRequest.call(
			...
			sandboxId,
			'POST',
			TOOLBOX_ENDPOINTS.process.execute,
			body,
